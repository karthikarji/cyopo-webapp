...
const DeleteUserModal: React.FC<Props> = ({ user, isDeleting, onConfirm, onClose }) => (
  <div className='fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4' onClick={onClose}>
    <div className='bg-surface rounded-2xl shadow-xl w-full max-w-sm p-6' onClick={(e) => e.stopPropagation()}>
      ...
      <h3 className='font-bold text-on-surface text-lg mb-2'>Delete user?</h3>
      <p className='text-sm text-on-surface-variant leading-relaxed mb-2'>
        This will permanently delete <strong>{user.name}</strong> and all their portfolios, messages and analytics. This cannot be undone.
      </p>
      ...
      <div className='flex items-center gap-3 mt-6'>
        <button
          ...
          disabled={isDeleting}
          className={[
            "flex-1 py-2.5 rounded-xl text-sm font-medium bg-error text-on-error transition-all",
