...
const BCTExperience: React.FC<{ experiences: Experience[] }> = ({ experiences }) => {
  ...
  const renderRow = (exp: Experience, i: number) => (
    <div
      key={exp.id ?? i}
      ...
      <div className='sm:text-right'>
        ...
        <p className='text-xs font-bold tracking-widest uppercase mt-0.5' style={{ color: "var(--tp)" }}>
          {exp.company}
        </p>
      </div>
    </div>
      ...
      <div className='max-w-7xl mx-auto'>
        ...
        {regular.length > 0 && (
          <>
            ...
            <div className='flex flex-col divide-y divide-white/5 mb-20'>{regular.map((exp, i) => renderRow(exp, i))}</div>
          </>
        )}
        ...
        {internships.length > 0 && (
          <>
            ...
            <div className='flex flex-col divide-y divide-white/5'>{internships.map((exp, i) => renderRow(exp, i))}</div>
          </>
        )}
      </div>
    </section>
